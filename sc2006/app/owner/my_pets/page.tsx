...
export default function MyPets() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans pb-20">
            ...
            {/* MODALS */}
            {isAddModalOpen && <AddPetModal onClose={() => setIsAddModalOpen(false)} />}
            {isDeleteModalOpen && <DeletePetModal pet={selectedPet} onClose={() => setIsDeleteModalOpen(false)} />}
        </div>
    );
}
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Pet's name" className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-teal-500" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Category *</label>
                        <select className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 focus:outline-none focus:border-teal-500">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" placeholder="e.g., Golden Retriever" className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-teal-500" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Age (years)</label>
                            <input type="number" placeholder="e.g., 3" className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-teal-500" />
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Vaccination Status</label>
                        <input type="text" placeholder="e.g., Up to date" className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-teal-500" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Special Needs / Notes</label>
                        <textarea rows={3} placeholder="Any special care requirements..." className="w-full px-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-teal-500 resize-none"></textarea>
                    </div>
                </div>
                ...
                <div className="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
                    ...
                    <button onClick={onClose} className="px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-medium transition-colors">Add Pet</button>
                </div>
            </div>
        </div>
    )
...
// DELETE PET MODAL
function DeletePetModal({ pet, onClose }: { pet: any, onClose: () => void }) {
